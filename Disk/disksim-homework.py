...
class Disk:
    ...
    # 2013011395 2013011409
    def Do_SCAN(self, rList):
        trackList = []
        ...
        while True:
            for (block, index) in rList:
                if self.requestState[index] == STATE_DONE:
                    continue
                angle = self.blockToAngleMap[index]
                if angle >= tempAngle and (angle - tempAngle) < minAngleDiff:
                    trackList = []
                    trackList.append((block, index))
                    minAngle = angle
                if angle == minAngle:
        ...
        while True:
            for (block, index) in rList:
                if self.requestState[index] == STATE_DONE:
                    continue
                angle = self.blockToAngleMap[index]
                if self.diresction == 1 and angle >= tempAngle and (angle - tempAngle) < minAngleDiff:
                    trackList = []
                    trackList.append((block, index))
                    nAngle = angle
                if self.diresction == -1 and angle <= tempAngle and (tempAngle - angle) < minAngleDiff:
                    trackList = []
                    trackList.append((block, index))
                    nAngle = angle
                if angle == nAngle:
    ...
    def GetNextIO(self):
        ...
        # 2013011395 2013011409
            ...
            trackList = self.Do_SCAN(self.requestQueue[0:self.GetWindow()])
            (self.currentBlock, self.currentIndex) = self.DoSATF(trackList)
        elif self.policy == 'CSCAN':
            trackList = self.Do_CSCAN(self.requestQueue[0:self.GetWindow()])
            (self.currentBlock, self.currentIndex) = self.DoSATF(trackList)
        else:
